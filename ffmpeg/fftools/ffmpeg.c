...
#ifdef _WIN32
typedef void (WINAPI * FN_FLUSH_MSWPFILESTREAM)(LPVOID);

static void* _mswpfileModuleHandle = 0;
static FN_FLUSH_MSWPFILESTREAM _fnFlushMemSwapFileStream = 0;

static void InitMemSwapFileModule()
{
    ...
    _fnFlushMemSwapFileStream = (FN_FLUSH_MSWPFILESTREAM)GetProcAddress(_mswpfileModuleHandle, "FlushMemSwapFileStream");
}
#endif
...
static int transcode(void)
{
    ...
    while (!received_sigterm) {
        ...
        for (i = 0; i < nb_output_files; i++) {
            ...
            uc = (URLContext*)os->pb->opaque;
            _fnFlushMemSwapFileStream(uc->priv_data);
        }
    }
#if HAVE_THREADS
    free_input_threads();
